...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                             ...
                             QCheckBox, QPushButton, QLabel, QSplitter, QMessageBox, QFileDialog,
                             QMenuBar, QMenu, QDialog, QComboBox, QDialogButtonBox)
from PyQt6.QtCore import Qt, QThread, pyqtSignal, QMimeData,QTimer,QSettings
from PyQt6.QtGui import QPalette, QColor, QFont, QDragEnterEvent, QDropEvent, QAction, QKeySequence,QIcon
...
def get_lang_dir() -> "pathlib.Path":
    if getattr(sys, 'frozen', False):              # 打包后
        base = pathlib.Path(sys._MEIPASS)          # exe 解压根目录
    else:                                          # 源码
        base = pathlib.Path(__file__).parent       # 脚本所在目录
...
def set_round_corners(w):
    if WIN:
        try:
            pws.set_round_corners(w, radius=12)
        except:
            pass
...
COMMAND_FIELDS = {
    ...
    "item":      ["Name", "Lore[]"],
    "execute":   ["run.title", "run.tellraw", "run.bossbar",
                  "run.team", "run.scoreboard", "run.item"],
}
...
def apply_mcfunction_translation(content: str, mapping, fname):
    ...
    for e in entries:
        ...
        # 解析路径后缀  line17.title.text  ->  ['title','text']
        ...
        # ② 新增：title / subtitle / actionbar / text  任意深度
...
class SettingsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.chk_vanilla = QCheckBox(tr("setting_show_vanilla"))
        self.chk_vanilla.setChecked(parent.show_vanilla)

        self.json_checks = {f: QCheckBox(f) for f in JSON_FIELDS}
        for f in JSON_FIELDS:
            self.json_checks[f].setChecked(f in parent.json_fields)

        self.cmd_checks = {c: QCheckBox(c) for c in COMMAND_TYPES}
...
class MainWindow(QMainWindow):
    ...
    def open_settings(self):
        ...
        if dlg.exec() == QDialog.DialogCode.Accepted:
            ...
            new_lang = dlg.cmb_lang.currentData()
    ...
    def save_dp(self):
        ...
        QMessageBox.information(self, "Done", tr("save_ok").format(out))

    def toggle_theme(self):
        self.dark = not self.dark
        QApplication.instance().setProperty("dark", self.dark)
        apply_theme(QApplication.instance(), self.dark)
    ...
    def load_recent(self, path):
        if pathlib.Path(path).exists():
            ...
            QMessageBox.warning(self, tr("tip"), tr("file_no_longer_exists\n{}").format(path))
    ...
    def close_current(self):
        if not self.zpath:
            QMessageBox.information(self, tr("tip"), tr("not_opened"))
            return
...
if __name__ == "__main__":
    ...
    # 默认语言文件若不存在则生成
    if not (LANG_PATH / "zh_CN.json").exists():
        (LANG_PATH / "zh_CN.json").write_text(json.dumps({
            "app_title": "MC 数据包翻译工具",
            "lang_name": "中文",
            "menu_file": "文件",
            "menu_open": "打开数据包",
    ...
    if not (LANG_PATH / "en_US.json").exists():
        (LANG_PATH / "en_US.json").write_text(json.dumps({
